...
# Вложенные классы для узлов и связей
class FrameNode:
...
class FrameConnection:
	...
	var stiffness: float = 0.9

var frame_nodes: Array[FrameNode] = []
var frame_connections: Array[FrameConnection] = []
	...
	for x in range(frame_resolution):
		for y in range(frame_resolution):
			for z in range(frame_resolution):
				...
				if x == 0 or x == frame_resolution - 1 or 
...
func create_connection(node_a_idx: int, node_b_idx: int):
	var connection = FrameConnection.new()
	connection.node_a = node_a_idx
	connection.node_b = node_b_idx
	connection.rest_length = frame_nodes[node_a_idx].position.distance_to(frame_nodes[node_b_idx].position)
	connection.stiffness = frame_stiffness
...
func apply_spring_force(connection: FrameConnection, delta):
