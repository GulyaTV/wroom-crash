...
## Максимально реалистичная визуальная и физическая деформация

@export var frame_resolution: int = 10  # Увеличенное разрешение для детализации
...
@export var visual_detail_multiplier: float = 2.0  # Множитель визуальной детализации
@export var metal_crease_threshold: float = 0.5  # Порог для создания складок
@export var surface_tension: float = 0.3  # Натяжение поверхности
@export var impact_propagation_speed: float = 15.0  # Скорость распространения удара

var frame_nodes: Array[Dictionary] = []
var frame_connections: Array[Dictionary] = []
var surface_triangles: Array[Dictionary] = []  # Треугольники поверхности
var deformation_history: Array[Dictionary] = []  # История деформаций
var impact_waves: Array[Dictionary] = []  # Волны от ударов
var original_mesh: BoxMesh
var car_body: RigidBody3D
var collision_forces: Array[Dictionary] = []
var mesh_cache: ArrayMesh = null  # Кэш меша для оптимизации
...
func _ready():
	...
	if mesh is BoxMesh:
		...
		print("Ultra BeamNG deformation initialized: ", frame_nodes.size(), " nodes, ", surface_triangles.size(), " triangles")
	else:
...
func create_ultra_frame_structure():
	...
	# Создаем узлы каркаса с повышенной плотностью
		...
		for y in range(frame_resolution):
			for z in range(frame_resolution):
				...
				if is_surface:
					...
					node["mass"] = 1.2  # Внутренние узлы тяжелее
				
				frame_nodes.append(node)
	
	# Создаем расширенные связи
	create_ultra_connections()
	
	# Создаем треугольники поверхности
	...
	for x in range(frame_resolution):
		for y in range(frame_resolution):
			for z in range(frame_resolution):
				...
				# Основные связи (6 направлений)
				var neighbors = [
					{"idx": current_idx + frame_resolution * frame_resolution, "cond": x < frame_resolution - 1},
					{"idx": current_idx - frame_resolution * frame_resolution, "cond": x > 0},
					{"idx": current_idx + frame_resolution, "cond": y < frame_resolution - 1},
					{"idx": current_idx - frame_resolution, "cond": y > 0},
					{"idx": current_idx + 1, "cond": z < frame_resolution - 1},
					{"idx": current_idx - 1, "cond": z > 0}
				]
				
				for neighbor in neighbors:
					if neighbor["cond"]:
						create_ultra_connection(current_idx, neighbor["idx"], 1.0)
				
				# Диагональные связи (12 направлений) для жесткости
				var diagonals = [
					{"dx": 1, "dy": 1, "dz": 0, "cond": x < frame_resolution - 1 and y < frame_resolution - 1},
					{"dx": 1, "dy": -1, "dz": 0, "cond": x < frame_resolution - 1 and y > 0},
					{"dx": -1, "dy": 1, "dz": 0, "cond": x > 0 and y < frame_resolution - 1},
					{"dx": -1, "dy": -1, "dz": 0, "cond": x > 0 and y > 0},
					{"dx": 1, "dy": 0, "dz": 1, "cond": x < frame_resolution - 1 and z < frame_resolution - 1},
					{"dx": 1, "dy": 0, "dz": -1, "cond": x < frame_resolution - 1 and z > 0},
					{"dx": -1, "dy": 0, "dz": 1, "cond": x > 0 and z < frame_resolution - 1},
					{"dx": -1, "dy": 0, "dz": -1, "cond": x > 0 and z > 0},
					{"dx": 0, "dy": 1, "dz": 1, "cond": y < frame_resolution - 1 and z < frame_resolution - 1},
					{"dx": 0, "dy": 1, "dz": -1, "cond": y < frame_resolution - 1 and z > 0},
					{"dx": 0, "dy": -1, "dz": 1, "cond": y > 0 and z < frame_resolution - 1},
					{"dx": 0, "dy": -1, "dz": -1, "cond": y > 0 and z > 0}
				]
				
				for diag in diagonals:
					if diag["cond"]:
						var diag_idx = (x + diag["dx"]) * frame_resolution * frame_resolution + 
									   (y + diag["dy"]) * frame_resolution + 
									   (z + diag["dz"])
						create_ultra_connection(current_idx, diag_idx, 0.7)  # Диагонали менее жесткие
...
func create_face_triangles(normal: Vector3, fixed_coord: int):
	...
	for x in range(resolution - 1):
		for y in range(resolution - 1):
			...
			# Определяем индексы в зависимости от грани
...
func apply_collision_deformation(collision_point: Vector3, collision_force: float, collision_normal: Vector3):
	...
	# Добавляем в историю деформаций
...
func apply_ultra_force_to_nodes(force_position: Vector3, force_amount: float, force_direction: Vector3):
	var influence_radius = force_amount * 25.0  # Увеличенный радиус влияния
	...
	for i in range(frame_nodes.size()):
		...
		if distance < influence_radius:
			...
			# Случайная компонента для реалистичности металла
...
func _physics_process(delta):
	...
	# Обновляем волны от ударов
	update_impact_waves(delta)
	
	# Ультра физика каркаса
	update_ultra_frame_physics(delta)
	
	# Обновляем визуальный меш с детализацией
...
func update_ultra_frame_physics(delta):
	var damping = 0.985  # Улучшенное затухание
	
	# Обновляем позиции и скорости узлов
	for i in range(frame_nodes.size()):
		var node = frame_nodes[i]
		
		if not node["fixed"]:
			# Обновляем ускорение
			node["acceleration"] *= 0.9  # Затухание ускорения
...
func update_ultra_visual_mesh(_delta):
	...
	# Генерируем ультра детализированные вершины
	...
	var colors = generate_deformation_colors()  # Цвета для визуализации деформации
...
func generate_ultra_vertices() -> PackedVector3Array:
	...
	# Генерируем вершины с детализацией для каждой грани
	var faces = [
		...
		for x in range(resolution):
			for y in range(resolution):
				...
				if node_idx < frame_nodes.size():
					...
					# Добавляем визуальную детализацию на основе деформации
					if node["deformation_level"] > 0.1:
						# Добавляем мелкие неровности
...
func get_face_node_index(x: int, y: int, fixed_coord: int, axis: String) -> int:
	...
	match axis:
		"y":
			if fixed_coord == resolution - 1:  # Верхняя грань
...
func calculate_ultra_normals(vertices: PackedVector3Array) -> PackedVector3Array:
	...
	# Улучшенные нормали с учетом деформации
	for face in range(6):
		...
		for i in range(face_vertex_count):
			...
			if vertex_idx < vertices.size():
				# Базовая нормаль грани
				var base_normal = get_face_normal(face)
				
				# Модификация на основе деформации
...
func generate_ultra_indices(vertex_count: int) -> PackedInt32Array:
	...
	# Генерируем индексы с возможностью subdivision
	for face in range(6):
		var face_offset = face * face_vertex_count
		for x in range(resolution - 1):
			for z in range(resolution - 1):
				...
				if i + resolution + 1 < vertex_count:
					...
					# Дополнительные треугольники для детализации (если высокая деформация)
					var base_node_idx = x * resolution * resolution + z
					if base_node_idx < frame_nodes.size():
						var deformation = frame_nodes[base_node_idx]["deformation_level"]
						if deformation > 0.3:
							# Добавляем дополнительные треугольники для детализации
							var center_idx = vertex_count  # Это потребует дополнительной логики
							indices.append(i)
	...
	for face in range(6):
		...
		for x in range(resolution):
			for z in range(resolution):
				var i = x * resolution + z + face_offset
				if i < vertex_count:
					# Базовые UV с возможностью искажения при деформации
					var base_uv = Vector2(x / float(resolution - 1), z / float(resolution - 1))
					
					# Искажение UV на основе деформации
...
func generate_deformation_colors() -> PackedColorArray:
	...
	for node in frame_nodes:
		var base_color = Color(0.2, 0.6, 1.0, 1.0)  # Базовый синий цвет
		
		# Модификация цвета на основе деформации
		var deformation = node["deformation_level"]
		var stress = node["stress_level"]
		
		if deformation > 0.1:
			# Деформированные участки становятся более красными
		...
		if stress > 0.5:
			# Высокий стресс делает участки более темными
			base_color *= (1.0 - stress * 0.3)
		
		# Складки добавляют желтоватый оттенок (металл)
		if node["crease_factor"] > 0.1:
...
func cleanup_old_data():
	...
	# Очищаем старую историю деформаций
...
func apply_damage_deformation(damage_level: float):
	var num_impacts = int(damage_level * 15) + 1  # Больше ударов при высоком уроне
	var size = original_mesh.size
